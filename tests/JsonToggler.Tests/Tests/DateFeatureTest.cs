...
{
    [TestClass]
    public class DateFeatureTest : BaseTestHelper
    {
        [TestInitialize]
        public void Setup()
        {
        ...
        public void Feature_CommandLessThanNow_IsEnabled_ReturnsTrue()
        {
            var jsonSettings = GetJsonSettings(EnvironmentEnum.LOCAL, PlatformEnum.Web);
            var feature = new DateFeature(jsonSettings);
            feature.Command = DateTime.UtcNow.AddMinutes(-1).ToString();
        ...
        [TestMethod]
        public void Feature_CommandGreaterThanNowByMinute_IsEnabled_ReturnsFalse()
        {
            var jsonSettings = GetJsonSettings(EnvironmentEnum.LOCAL, PlatformEnum.Web);
            var feature = new DateFeature(jsonSettings);
            feature.Command = DateTime.UtcNow.AddMinutes(1).ToString();
